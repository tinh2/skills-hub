...
export async function installRoutes(app: FastifyInstance) {
  ...
  app.post<{ Params: { slug: string } }>("/:slug/install", writeRateLimit, async (request) => {
    const user = await optionalAuth(request);
    const parsed = recordInstallSchema.safeParse(request.body);
    if (!parsed.success) throw new ValidationError(parsed.error.issues[0].message);
    const ip = request.ip;
    await installService.recordInstall(request.params.slug, parsed.data, user?.userId ?? null, ip);
    return { success: true };
  });
}

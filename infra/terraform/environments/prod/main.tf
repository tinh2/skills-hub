...
resource "aws_security_group" "database_public" {
  ...
  name_prefix = "skills-hub-prod-db-public-"
  vpc_id      = data.aws_vpc.default[0].id

  # SECURITY NOTE: 0.0.0.0/0 required because App Runner in non-VPC mode has no
  # fixed egress IPs. RDS is protected by password auth + TLS. Switch to use_vpc = true
  # for private networking when the project has paying users.
  ingress {
    description = "PostgreSQL from App Runner (non-VPC mode)"
    from_port   = 5432
    to_port     = 5432
    protocol    = "tcp"
  ...
  tags = {
    Name = "skills-hub-prod-database-public"
  }
}
